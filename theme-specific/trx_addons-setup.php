...
if ( ! function_exists( 'gutentype_trx_addons_slider_title' ) ) {
	...
	function gutentype_trx_addons_slider_title( $title, $data ) {
		...
		if(!empty($data['date']) || !empty($data['id']) ){
			...
			$reading_time_multilang = array(
						  "en" => "minutes read",
						  "al" => "minuta lexim"
			);
			$lang_string = (isset($_GET["lang"])) ?  $reading_time_multilang["en"] : $reading_time_multilang["al"];
			$reading_time_text = (get_post_meta($data['id'], 'reading_time', true)) ? '<i class="icon-04-time" style="    font-size: 15px;
    font-style: normal;
    color: #FFFFFF!important;
    margin-right: 5px;"></i><span style="margin-right:10px; color:#ffffff;"> ' . get_post_meta($data['id'], 'reading_time', true)  . ' ' . $lang_string . '.</span>  ' : "";

                        $title .=  $reading_time_text .
